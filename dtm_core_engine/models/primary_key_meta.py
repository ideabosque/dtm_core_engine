    ...
    def list(
        ...
        *,
        ...
        if associated_model_uuid:
            ...
            if not docs:
                for _k, payload in etcd_client.iter_prefix(f"{ETCD_PREFIX}/primary_key_metadata/{associated_model_uuid}/"):
                    if payload:
                        docs.append(payload)
        else:
...
def resolve_primary_key_meta(info: ResolveInfo, **kwargs: Dict[str, Any]) -> PrimaryKeyMetaType:
...
def resolve_primary_key_meta_list(info: ResolveInfo, **kwargs: Dict[str, Any]) -> PrimaryKeyMetaListType:
    repo = PrimaryKeyMetaRepo()
    metas = repo.list(
        associated_model_uuid=kwargs.get("associated_model_uuid"),
        attribute_name=kwargs.get("attribute_name"),
    ...
    meta_list = [
        PrimaryKeyMetaType(**repo._process_datetime_fields(m))
        for m in metas
    ]
    return PrimaryKeyMetaListType(primary_key_meta_list=meta_list, total=len(metas))


def insert_update_primary_key_meta(info: ResolveInfo, **kwargs: Dict[str, Any]) -> PrimaryKeyMetaType:
    repo = PrimaryKeyMetaRepo()
    if "primary_key_metadata_uuid" not in kwargs:
        kwargs["primary_key_metadata_uuid"] = etcd_client.new_id()
    ...
    idx_key = k_idx_associated_pkmeta(kwargs["associated_model_uuid"], kwargs["primary_key_metadata_uuid"])
